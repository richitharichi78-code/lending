...
class TestLoanImportTool(IntegrationTestCase):
	...
	def run_tool_import(self, tool_doc):
		target_doctype = tool_doc.get_target_doctype()

		parsed_file = ImportFile(target_doctype, tool_doc.import_file, import_type="Insert New Records")
		payload_count = len(parsed_file.get_payloads_for_import())

		di = tool_doc.create_data_import(
		...
		tool = frappe.get_doc(
			{
				...
				"import_for": "Loan",
				"loan_import_type": "Mid Tenure Loans",
				"import_file": loan_file,
			}
		).insert(ignore_permissions=True)

		self.run_tool_import(tool)

		self.assertTrue(frappe.db.exists("Loan", loan_id))
		self.assertTrue(frappe.db.exists("Loan Disbursement", {"against_loan": loan_id}))
...
{repayment_id},{loan_id},{disbursement_id},Normal Repayment,2024-07-15,2024-07-15,50000,40000,8000,1200,800,0,0,0,Payment Account - _TC,Loan Account - _TC,,REF-{random_string(8)},2024-07-15,Test repayment"""
		...
		self.assertTrue(frappe.db.get_value("Loan Repayment", repayment_id, "is_imported"))
...
{repayment_ids[2]},{loan_id},,Normal Repayment,2024-05-10,2024-05-10,122000,115000,7000,0,0,0,0,0,Payment Account - _TC,Loan Account - _TC,,REF-3-{random_string(8)},2024-05-10,Final"""
		...
		repayment_file = self.create_test_csv_file(repayment_csv, "closed_repayments.csv")
		...
		repayment_count = frappe.db.count("Loan Repayment", {"against_loan": loan_id, "is_imported": 1})
